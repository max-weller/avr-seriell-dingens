		...
		if (recvidx == -3) { // incoming message in recvpkg
			...
			if (recvpkg[PI_LENGTH] & (PF_ACK|PF_ERROR)) {
				...
				case 0x50: //set constcolor
					ws2812b_constcolor(recvpkg[3], recvpkg[4], recvpkg[5], recvpkg[6]);
				...
				case 0x51: //set bmp rgb range
					j = PI_DATA+1;
					i = recvpkg[PI_DATA];
					while (j<recvpkg[PI_LENGTH] && i<STRIPE_LENGTH) {
						bmp_r[i] = recvpkg[j++];
						bmp_g[i] = recvpkg[j++];
						bmp_b[i] = recvpkg[j++];
						i++;
				...
				case 0x53: //dim/lighten bmp range
					i = recvpkg[PI_DATA];
					if (i >= STRIPE_LENGTH || recvpkg[PI_DATA+1] >= STRIPE_LENGTH || recvpkg[PI_DATA+1] < i) {
						rs485_error(); break;
					}
					while(i<=recvpkg[PI_DATA+1]) {
						if (recvpkg[PI_CMD] == 0x52) {
							bmp_r[i] = recvpkg[PI_DATA+2];
							bmp_g[i] = recvpkg[PI_DATA+3];
							bmp_b[i] = recvpkg[PI_DATA+4];
						} else {
							bmp_r[i] += (int8_t)recvpkg[PI_DATA+2];
							bmp_g[i] += (int8_t)recvpkg[PI_DATA+3];
							bmp_b[i] += (int8_t)recvpkg[PI_DATA+4];
						}
						i++;
					}
					rs485_ack(0xAA);
				...
				case 0x55: //scroll bmp range right
					i = recvpkg[PI_DATA+1];
					if (recvpkg[PI_DATA] >= STRIPE_LENGTH || i >= STRIPE_LENGTH || recvpkg[PI_DATA] > i) {
						rs485_error(); break;
					}
					j = bmp_r[i];
					k = bmp_g[i];
					l = bmp_b[i];
					while(i>recvpkg[PI_DATA]) {
						bmp_r[i] = bmp_r[i-1];
						bmp_g[i] = bmp_g[i-1];
						bmp_b[i] = bmp_b[i-1];
						i--;
